...
import {CallOptions, IGatewayZEVM} from "./IGatewayZEVM.sol";
...
import {MessageContext, UniversalContract} from "./UniversalContract.sol";
    ...
    INotSupportedMethods
{
    ...
    address public constant PROTOCOL_ADDRESS =
    ...
    receive() external payable whenNotPaused {
        if (msg.sender != zetaToken && msg.sender != PROTOCOL_ADDRESS)
            revert OnlyWZETAOrProtocol();
    }
    ...
    function _withdrawZRC20WithGasLimit(
        ...
        if (
            !IZRC20(gasZRC20).transferFrom(msg.sender, PROTOCOL_ADDRESS, gasFee)
        ) {
    ...
    function _transferZETA(uint256 amount, address to) private {
        if (!IWETH9(zetaToken).transferFrom(msg.sender, address(this), amount))
    ...
    function withdraw(
        ...
        emit Withdrawn(
            ...
            "",
            CallOptions({
                gasLimit: IZRC20(zrc20).GAS_LIMIT(),
    ...
    function withdrawAndCall(
        ...
        if (
            message.length + revertOptions.revertMessage.length >
            MAX_MESSAGE_SIZE
        ) revert MessageSizeExceeded();

        uint256 gasFee = _withdrawZRC20WithGasLimit(
            amount,
            zrc20,
    ...
    function deposit(
        ...
        uint256 amount,
        address target
    ) external onlyProtocol whenNotPaused {
        if (zrc20 == address(0) || target == address(0)) revert ZeroAddress();
        if (amount == 0) revert InsufficientZRC20Amount();

        if (target == PROTOCOL_ADDRESS || target == address(this))
