    ...
    INotSupportedMethods
{
    ...
    bytes32 public constant ASSET_HANDLER_ROLE =
        keccak256("ASSET_HANDLER_ROLE");
        ...
        returns (bytes memory)
    {
        ...
        if (messageContext.sender == address(0)) {
            ...
            result = _executeAuthenticatedCall(
                messageContext,
                destination,
                data
            );
    ...
    function executeWithERC20(
        ...
        uint256 amount,
        bytes calldata data
    ) public nonReentrant onlyRole(ASSET_HANDLER_ROLE) whenNotPaused {
    ...
    function revertWithERC20(
        ...
        bytes calldata data,
        RevertContext calldata revertContext
    ) external nonReentrant onlyRole(ASSET_HANDLER_ROLE) whenNotPaused {
        if (amount == 0) revert InsufficientERC20Amount();
        if (to == address(0)) revert ZeroAddress();
    ...
    function _transferFromToAssetHandler(
        ...
        if (token == zetaToken) {
            ...
            // transfer to custody
            if (!IERC20Custody(custody).whitelisted(token))
                revert NotWhitelistedInCustody();
    ...
    function _transferToAssetHandler(address token, uint256 amount) private {
        if (token == zetaToken) {
            // transfer to connector
            // approve connector to handle tokens depending on connector version (eg. lock or burn)
            if (!IERC20(token).approve(zetaConnector, amount))
    ...
    function _executeAuthenticatedCall(
        ...
        return
            Callable(destination).onCall{value: msg.value}(
                messageContext,
                data
            );
    }
