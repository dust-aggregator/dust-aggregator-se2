 ...
 */
export const useDustEventHistory = <
...
>({
  ...
  const isContractAddressAndClientReady = Boolean(outputNetwork?.contractAddress) && Boolean(publicClient);
  ...
  const query = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      ...
      const data = await getEvents(
        { address: outputNetwork?.contractAddress, event, fromBlock: pageParam, args: filters },
        publicClient,
